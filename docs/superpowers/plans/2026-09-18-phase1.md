...
- [x] **Step 3: Two short runs** (§9 boxes 2, 3, 5, 7)
...
- [x] **Step 6: README usage and the Go versus C# section** (§9 box 10)
...
## Task 15: Burst within the server's hourly count (DD-11)

A follow-up to Task 7, added 2026-09-19 after the first real imports. Even spacing (one request every 36 s) made a `--budget 5` run take about 2½ minutes and made re-imports crawl. It bought nothing in throughput, because the hourly cap binds either way. See PRD DD-11 for the decision and its reasoning.

**Files:**
- Create: `internal/source/hourly.go`, `internal/source/hourly_test.go`
- Modify: `internal/source/client.go`, `internal/source/client_test.go`
- Modify: `internal/source/pokewallet/pokewallet.go`, `internal/source/pokewallet/pokewallet_test.go`
- Modify: `cmd/pricewatch/providers.go`, `cmd/pricewatch/import.go`, `cmd/pricewatch/run.go` (pass the logger through)
- Modify: `README.md` (rate-limits section)

**Interfaces:**
- Consumes: `source.Config`, `source.Limits`, `card.ErrRateLimited`
- Produces:
  - `source.Config.HourCount func(http.Header) (limit, remaining int, ok bool)`. A nil value keeps today's even spacing.
  - `source.Config.Log *slog.Logger`. When set, the client logs whenever it pauses for the hourly window, so a long pause is never mistaken for a hang.
  - `func HeaderCount(limitHeader, remainingHeader string) func(http.Header) (int, int, bool)`
  - `pokewallet.Limits` becomes `{PerSecond: 2, PerHour: 100, PerDay: 1000}`, and `pokewallet.Config` sets `HourCount`.
  - `providerOpts.Log *slog.Logger`, copied into the provider's `source.Config`.
  - Package-internal: `type hourly`, with `reserve(now) time.Duration`, `observe(sentAt, limit, remaining)` and `release()`.

How it works:
- **`hourly` holds the server's count.** It is re-synced from every response, minus the requests still in flight, so concurrent workers can't overshoot between responses.
- **`reserve`** returns 0 while requests remain, or how long to wait once none do.
- **The wait lasts until one window after the current window's first request.** The first request of a window is the one whose response shows `remaining = limit - 1`; failing that, it is the first request this process made. That is safe whether the server counts a fixed clock hour or a rolling hour, and safe after a restart.
- **A request that gets no count back**, such as a network error, frees its in-flight slot but does not give the count back, because the server may still have counted it.

> **Go note:** `hourly` is a small struct guarded by a `sync.Mutex`, and all the timing logic takes `now` as a parameter. That makes it testable with plain `time.Time` values: no sleeping, no fake clock, no HTTP. The client only has to wait for the duration `reserve` returns, which it does with a `select` on a timer and `ctx.Done()`, so Ctrl-C interrupts the wait.

- [x] **Step 1: Write the failing tests**

```go
// internal/source/hourly_test.go
package source
...
Append to `internal/source/client_test.go`, adding `"bytes"`, `"log/slog"`, `"strconv"` and `"strings"` to its imports:
...
Append to `internal/source/pokewallet/pokewallet_test.go`, adding `"net/http"` and `"time"` to its imports:
...
Run: `go test ./internal/source/...`
Expected: a build failure (`undefined: newHourly`, `HeaderCount`, `pacing`, and unknown fields `HourCount` and `Log`).

- [x] **Step 3: Implement**

```go
// internal/source/hourly.go
package source
...
In `internal/source/pokewallet/pokewallet.go`:
...
and add to the `source.Config` literal in `Config`:

```go
		HourCount: source.HeaderCount("X-RateLimit-Limit-Hour", "X-RateLimit-Remaining-Hour"),
```

In `cmd/pricewatch/providers.go`, add `Log *slog.Logger` to `providerOpts`, and `cfg.Log = o.Log` before `source.New(cfg)`. In `cmd/pricewatch/import.go` and `cmd/pricewatch/run.go`, set `o.Provider.Log = log` next to `o.Provider.Quota = st`.

- [x] **Step 4: Run the tests and confirm they pass**

Run: `go test ./...` then `go test -count=20 -run 'Burst|WaitsOut|Honours|Pacing|Hourly' ./internal/source/`
Expected: `ok`, every time.

- [x] **Step 5: Update the README's rate-limits section**

Replace the "spaces requests to one every 36 seconds" bullet with the DD-11 behaviour:
- requests go out immediately while PokéWallet reports requests left this hour, at most 2 per second;
- once none are left, the run pauses until the hour is certainly over, logs that it is waiting, and can be stopped with Ctrl-C;
- a 100-request run takes about a minute.

- [x] **Step 6: Commit**

```bash
git add internal/source cmd/pricewatch README.md docs/superpowers/plans/2026-09-18-phase1.md
git commit -m "feat(source): burst within the server's hourly count, wait out the window (DD-11)"
```

Rebuild `pricewatch.exe` only once no import or run is using it. Windows locks a running executable.

**Follow-up (2026-09-18, after the first real run of this task).** The first import with bursting hit a 429 at exactly 100 requests. PokéWallet's `X-RateLimit-Remaining-*` headers report the count from *before* the carrying request, while the client assumed *after*. The fix lives in the provider, not the shared client: `pokewallet.Config` now uses `hourCount` and `dayUsed`, which subtract the carrying request.

Tests:
- `TestConfigCountsTheRequestCarryingTheHeaders` checks the conversion.
- `TestClientNeverOverspendsPokeWalletsHourlyLimit` runs the real client against a fake server that counts the same way and enforces a hard limit, and checks the next request waits instead of being sent.

**Follow-up 2 (2026-09-18): a 429 on the first request is waited out.** A process that starts mid-hour knows no count until its first response, so a spent hour meant an immediate 429 and a stopped run. `GetJSON` now wraps a single attempt (`getOnce`). A 429 whose headers show the hour spent but the day not is marked `hourlyLimited`, waited out through `awaitHour`, and retried up to `maxHourlyRetries` (3) times. It still unwraps to `card.ErrRateLimited`, so callers see the same error if the retries run out.

Tests:
- `TestHourly429IsWaitedOutAndRetried` checks the wait and the retry.
- `TestA429ThatIsNotHourlyStillStops` covers a spent day and a 429 with no counts.
- `TestHourlyRetriesAreCapped` checks the retry limit.

**Follow-up 3 (2026-09-19): PokéWallet resets on the clock hour.** One request at 00:06, after the allowance ran out at 23:29, showed it full again. A rolling window would still have been spent until about 00:28. PokéWallet sends no reset header.

Changes:
- `source.Config.HourWindow` is a new per-source setting. `Rolling`, the zero value, keeps the conservative rule. `ClockHour` waits until the next UTC hour boundary plus `clockMargin` (30 s).
- `hourly` records `spentAt`, when the window was found spent, and `resetAt()` picks the rule.
- `pokewallet.Config` sets `ClockHour`.

Tests:
- `TestClockHourWaitsForTheNextHourBoundary` covers six scenarios, including spent at :29, spent seconds before the hour, and a restart.
- `TestNewPicksTheSourcesHourWindow` checks the wiring.
- `TestConfigResetsOnTheClockHour` checks PokéWallet's setting.
...
| FR-3 rate limit (hourly allowance from the server, durable daily quota) | 5 (quota store), 7, 15 (DD-11) |
