...
func TestParse(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			body:     `International,Mudkip,59/109,59,"EX Ruby & Sapphire, Promo",Common,Normal,English,,,,` + "\n",
			wantRows: 1,
			check: func(t *testing.T, rows []cardRow) {
				r := rows[0]
...
- [ ] **Step 5: Smoke-test against the real export (local only, nothing committed)**
...
- [ ] **Step 5: Run the live test once (5 real requests)**
...
func TestResolve(t *testing.T) {
	tests := []struct {
		...
		{"ambiguous expansion", r("Pikachu", "Promos", "1", "Normal", "English"), nil,
			card.StatusAmbiguous, "", "", "expansion matches sets"},
...
- [ ] **Step 5: Add a compile-time check that the provider satisfies the interface**
...
Then:
1. Copy the suggested `expansion,set_id` pairs from the `unknown expansion … (candidates: …)` lines into `data/expansions.csv`, checking each by eye.
2. Re-run with `--expansions data/expansions.csv`. Only previously failed keys are retried.
